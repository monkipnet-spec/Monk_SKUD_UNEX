...
std::string row(const User&u){ std::ostringstream o; o<<u.id<<';'<<(u.enabled?1:0)<<';'<<clean(u.last_name)<<';'<<clean(u.first_name)<<';'<<clean(u.middle_name)<<';'<<clean(u.department)<<';'<<clean(u.position)<<';'<<clean(u.card)<<';'<<u.controller_port<<';'<<clean(u.valid_from)<<';'<<clean(u.valid_until)<<';'<<(u.telegram_arrival?1:0)<<';'<<(u.telegram_departure?1:0); return o.str(); }
}
UserManager::UserManager(std::string path):path_(std::move(path)){}
bool UserManager::load(){std::lock_guard lk(mu_);users_.clear();std::ifstream f(path_);if(!f)return false;std::string line;bool first=true;while(std::getline(f,line)){if(first){first=false;continue;}auto c=util::split(line,';');if(c.size()<12)continue;try{User u;u.id=std::stoi(c[0]);u.enabled=c[1]!="0";u.last_name=c[2];u.first_name=c[3];u.middle_name=c[4];u.department=c[5];u.position=c[6];u.card=c[7];if(c.size()>=13){u.controller_port=c[8].empty()?0:std::stoi(c[8]);u.valid_from=c[9];u.valid_until=c[10];u.telegram_arrival=c[11]!="0";u.telegram_departure=c[12]!="0";}else{u.controller_port=0;u.valid_from=c[8];u.valid_until=c[9];u.telegram_arrival=c[10]!="0";u.telegram_departure=c[11]!="0";}users_.push_back(u);}catch(...){}}return true;}
bool UserManager::save()const{std::lock_guard lk(mu_);std::filesystem::create_directories(std::filesystem::path(path_).parent_path());auto tmp=path_+".tmp";std::ofstream f(tmp,std::ios::trunc);if(!f)return false;f<<"id;enabled;last_name;first_name;middle_name;department;position;card;controller_port;valid_from;valid_until;telegram_arrival;telegram_departure\n";for(auto&u:users_)f<<row(u)<<"\n";f.close();std::error_code ec;std::filesystem::rename(tmp,path_,ec);if(ec){std::filesystem::remove(path_,ec);ec.clear();std::filesystem::rename(tmp,path_,ec);}return !ec;}
...
bool UserManager::renameDepartment(const std::string&old_name,const std::string&new_name){
    if(old_name==new_name)return true;
    {
        std::lock_guard lk(mu_);
        for(auto&u:users_)if(u.department==old_name)u.department=new_name;
    }
    return save();
}
bool UserManager::departmentInUse(const std::string&name)const{
    std::lock_guard lk(mu_);
    return std::any_of(users_.begin(),users_.end(),[&](const auto&u){return u.department==name;});
}
std::vector<std::string>UserManager::usedDepartments()const{
    std::lock_guard lk(mu_);
    std::vector<std::string> out;
    for(const auto&u:users_){
        auto name=util::trim(u.department);
...
std::string UserManager::exportCsv()const{std::lock_guard lk(mu_);std::ostringstream o;o<<"id;enabled;last_name;first_name;middle_name;department;position;card;controller_port;valid_from;valid_until;telegram_arrival;telegram_departure\n";for(auto&u:users_)o<<row(u)<<"\n";return o.str();}
