...
bool App::init(){
    ...
    store_=std::make_unique<FileStore>(root_);users_=std::make_unique<UserManager>(root_+"/config/users.csv");if(!users_->load()){users_->save();}
    ...
    web_=std::make_unique<WebServer>(*cfg_,*users_,*attendance_,*controllers_,*telegram_,root_);
    return true;
}
