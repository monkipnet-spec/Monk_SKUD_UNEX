...
bool TelegramNotifier::sendText(const std::string&text,std::string&err){
    ...
    if(pid==0){execlp("curl","curl","-fsS","--max-time","10","-X","POST","--data-urlencode",("chat_id="+chat).c_str(),"--data-urlencode",("text="+text).c_str(),url.c_str(),(char*)nullptr);_exit(127);}
    int st=0;if(waitpid(pid,&st,0)<0){err="waitpid failed";return false;}if(WIFEXITED(st)&&WEXITSTATUS(st)==0)return true;err="curl exit code "+std::to_string(WIFEXITED(st)?WEXITSTATUS(st):-1);return false;
}
