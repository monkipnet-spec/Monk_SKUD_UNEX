## v0.3.39 — full RS-485 controller discovery on refresh

- Исправлено обнаружение нескольких контроллеров: раньше диапазон `controllers.scan_from..controllers.scan_to` сканировался только когда в локальной базе не было ни одного включённого контроллера. После обнаружения первого устройства новые Node ID больше не искались.
- Кнопка ручного обновления контроллеров и первое открытие COM-порта теперь выполняют безопасный полный `24H Read Device RTC`-опрос всего настроенного диапазона Node ID. Уже известные контроллеры обновляются, новые автоматически добавляются и сохраняются.
- Обычный фоновый polling не сканирует весь диапазон постоянно; полный поиск запускается только по refresh/открытию порта либо когда список контроллеров пуст.
...
С **v0.3.38** доступен **расширенный отчёт по пользователям**: можно выбрать одного, несколько или всех пользователей и произвольный период. В отличие от обычного суточного агрегата он выводит **каждое сохранённое событие `arrival` и `departure`** в хронологическом порядке, включая время, карту и контроллер. Расширенный отчёт можно просмотреть, скачать TXT или отправить TXT в Telegram.
