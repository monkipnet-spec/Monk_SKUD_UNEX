## v0.3.16 — disable Pass Any Cards + forced 83H full reconciliation

- По реальному 25H событию подтверждён характерный симптом H-series `Pass Any Cards`: неизвестная/изменённая карта получает `Normal Access` с `user=1023`.
- Перед полной синхронизацией программа читает EEPROM `0x0016` (Assigned function / `24*`), снимает только `0x20` (`Enable swipe any tags to release door open`), записывает байт через `20H` и подтверждает `12H` read-back. Остальные биты параметра сохраняются.
- Полная синхронизация больше не пропускает адрес, если `87H` уже показывает нули: каждый отсутствующий slot 0..1023 обязательно получает реальную `83H` запись из 8 нулей и контрольный `87H`, чтобы обновить внутреннюю таблицу/кэш доступа UNEX 721.
- Исправлены оставшиеся web-проверки диапазона Address 0..1023 и устаревшее предупреждение про `85H`.
- Cache revision: `v=0316`.

## v0.3.15 — verified full sync + fixed site header

- Полная синхронизация больше не полагается на `85H`: контроллер может подтвердить `85H`, но оставить старые активные слоты. Теперь программа проходит весь официальный диапазон `0..1023`, отключает отсутствующие в локальной базе слоты через `83H` и подтверждает каждое изменение через `87H`, затем записывает нужных пользователей с тем же read-back verification.
- Шапка сайта закреплена сверху (`position: sticky`) и остаётся видимой при прокрутке страницы.
- Cache revision web UI обновлена до `v=0315`.
...
- Каждая записанная пользовательская ячейка по-прежнему обязательно проверяется командой `87H`. Если `85H` не подтверждена ACK, запись на этот контроллер не начинается.
- Это удаляет старые и дублирующиеся карты из слотов, которых нет в текущей выгружаемой базе. Для выборочной выгрузки `85H` не используется.
...
## v0.3.34 — закреплённые активные строки MAG250

- В MAG250-портале сотрудники со статусом «НА РАБОТЕ» и «УШЁЛ» закреплены сверху и не участвуют в циклической прокрутке.
- Закреплённая часть сортируется по статусу: сначала «НА РАБОТЕ», затем «УШЁЛ», внутри статуса — по ФИО.
- Нижняя прокручиваемая часть содержит отсутствующих/отключённых сотрудников.
- «НЕ ПРИХОДИЛ» переименовано в «ОТСУТСТВУЕТ».
- Скорость автопрокрутки нижнего списка увеличена в 2 раза (с 2 до 4 px за шаг).
- Шрифт закреплённых строк «НА РАБОТЕ» и «УШЁЛ» увеличен для телевизионного отображения.
...
## v0.3.35 — динамическая ширина колонок MAG250

- В MAG250-портале колонки `Должность`, `Отдел`, `Приход`, `Уход` теперь рассчитываются по самой широкой фактической записи (с учётом заголовка и внутренних отступов).
- Освободившаяся ширина автоматически передаётся колонке `Сотрудник`, чтобы длинные ФИО реже обрезались.
- Для строк `На работе` и `Ушёл` расчёт учитывает их увеличенный шрифт.
- При изменении списка сотрудников или разрешения экрана ширины пересчитываются автоматически.
- Закреплённые `На работе` / `Ушёл`, статус `Отсутствует` и ускоренная прокрутка из v0.3.34 сохранены.


## v0.3.36 — серверное время в MAG250

- Часы и дата в `/mag250/` синхронизируются по полю `generated_at` ответа `/api/monitor`, которое формирует сервер Monk_SKUD_UNEX.
- Локальные дата, время и часовой пояс MAG250 больше не используются для отображаемых часов.
- Между 3-секундными запросами часы продолжают идти каждую секунду от последней серверной синхронизации.
- Разбор времени сделан ES5-совместимо; отображение использует UTC-accessors только как способ сохранить серверные компоненты даты/времени независимо от timezone приставки.
