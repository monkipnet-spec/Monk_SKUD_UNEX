...
- polling UNEX/SOYAL-style протокола: 0x25 (старейшее событие) и 0x37; декодированные карты передаются в учёт, неизвестный RAW перед удалением сохраняется в quarantine-файл;
...
- пользователи, несколько карт на одного пользователя (каждая карта = серия + номер) и индивидуальный PIN в `config/users.csv`;
...
- текущее состояние присутствия `data/card_state.csv` (v0.3.3: состояние хранится на пользователя, чтобы несколько его карт работали как одна учётная запись);
...
Неизвестные варианты event-frame не угадываются: полный RAW сохраняется в `data/events/undecoded_unex.csv`, после чего 0x37 освобождает FIFO, чтобы старая запись не блокировала новые проходы.
